...
function M.parse(content)
    ...
    for i,setting in ipairs(out) do
        if setting.targetIds then
            ...
            setting.targets={}
            for _,id in ipairs(setting.targetIds) do
                ...
                assert(child.kind=="picker" and not child.targetIds and not child.targets,"preset targets must be ordinary pickers")
                for _,value in ipairs(setting.values) do
                    assert(value==setting.custom or M.index(child,value),"preset value missing from target choices")
                end
                owners[target]=i
...
function M.open(provider)
    ...
    local function fill(values,setting,value)
        if setting.targets and value~=setting.custom then
            for _,target in ipairs(setting.targets) do values[target]=value end
        end
    end
    local ok,err=pcall(function()
        ...
        for i,setting in ipairs(model.items) do fill(model.pending,setting,model.pending[i]) end
